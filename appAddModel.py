            ...
            if apiObj:
                ...
                if user_input.strApi.lower() == "openai":
                    ...
                    for item in message.content[0]:
                        if item[0]=='text':
                          strRawOutput += item[1]
                          response = {"message": sanitize_input(item[1]), "element_id": element_id}
                          yield f"data: {json.dumps(response)}\n\n"
    ...
    try:
        ...
        if request.method == 'GET':
                        ...
                        if postProcResp:
                            ...
                            if aiShieldsReport:
                                ...
                                if apiResponse:
                                    ...
                                    return render_template('chat.html', inputprompt=rawInputStr,recalloutput='  Raw Output from '+ apiStr + ' : '+ rawOutputStr + '\n\n AiShields Post Processed Output \n\n' + postProcRespStr,rawInput=rawInput.inputPrompt, preProcStr=preprocPrompt.preProcInputPrompt, rawResponse=rawOutputStr, InputPromptHistory=chathistory, PostProcResponseHistory=postProcRespStr, apis=apis, email=session['email'], username=session['username'], response=postProcRespStr, findings=findings, output=True, logged_in=True)
            ...
            if user_input != 'None' and user_api != 'None': # and user_tkn:
                ...
                return Response(stream_with_context(generate_response(user_info)), mimetype='text/event-stream')
            return render_template('chat.html', findings=[],InputPromptHistory=chathistory, apis=apis, email=session['email'], username=session['username'], logged_in=True) 
...
def aishields_add_user():
    try:
        user2add = User(username=str('Obakeng Mothibedi'),first_name='Obakeng',last_name='Mothibedi',email='obakengmothibedi@gmail.com',passphrase=getHash('Welcom3OM!'),user_verified=1,subscribed=1,created_date=datetime.datetime.now(datetime.timezone.utc))
...
def aishields_add_model():
    try:
        model2add = GenApi(api_owner=str('Anthropic'),api_name='Claude',uri='https://api.anthropic.com/v1/messages',headers='',formfields='',model='claude-3-5-sonnet-20240620',created_date=datetime.datetime.now(datetime.timezone.utc),updated_date=datetime.datetime.now(datetime.timezone.utc))
        # user2add.first_name = 'Obakeng'
        # user2add.last_name = 'Mothibedi'
        # user2add.email = 'obakengmothibedi@gmail.com'
        # user2add.username = 'Obakeng Mothibedi'
        ...
        db.session.commit()
        db.session.flush(objects=[model2add])
    except Exception as err:
